  ...
  return (
    <RequirePerms modulo={CFG.moduleSlug} accion="ver">
      <ListView
        schema={schema}
        data={customers}
        onViewRow={(row) => router.push(`/system/${CFG.moduleSlug}/${row.id}`)}
        onEditRow={(row) => router.push(`/system/${CFG.moduleSlug}/${row.id}?edit=true`)}
        onDeleteRow={handleDelete}
        onCreate={() => router.push(`/system/${CFG.moduleSlug}/new`)}
      />
    </RequirePerms>
  );
}
