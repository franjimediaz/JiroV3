...
export default function CustomersPageClient({
  ...
  const handleDelete = async (row: any) => {
    // 1) permiso (UX)
    if (!hasPermiso("services", "eliminar")) {
      router.replace("/403");
      return;
    }
  ...
  return (
    <RequirePerms modulo="services" accion="ver">
      <ListView
        schema={schema}
        data={customers}
        onViewRow={(row) => router.push(`/system/services/${row.id}`)}
        onEditRow={(row) => router.push(`/system/services/${row.id}?edit=true`)}
        onDeleteRow={handleDelete}
        onCreate={() => router.push("/system/services/new")}
      />
    </RequirePerms>
  );
}
