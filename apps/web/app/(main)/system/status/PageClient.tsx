...
export default function CustomersPageClient({
  ...
  const handleDelete = async (row: any) => {
    ...
    const { error } = await supabase.from("status").delete().eq("id", row.id);
  ...
  return (
    <RequirePerms modulo="status" accion="ver">
      <ListView
        schema={schema}
        data={customers}
        onViewRow={(row) => router.push(`/system/status/${row.id}`)}
        onEditRow={(row) => router.push(`/system/status/${row.id}?edit=true`)}
        onDeleteRow={handleDelete}
        onCreate={() => router.push("/system/status/new")}
      />
    </RequirePerms>
  );
}
